...
const generateLevel = (levelIdx) => {
  ...
  while(!valid) {
    ...
    while(heads.length > 0 && iters < 50) {
      ...
      let nextDir = ptype === 'reflector_ne' ? DEFLECT_NE[cdir] : DEFLECT_NW[cdir];
      
      if (levelIdx > 0 && Math.random() > 0.7) {
        ptype = 'bifurcator';
      }
      ...
      if (levelIdx > 1 && Math.random() > 0.8 && !pieces.find(p=>p.type==='portal_blue')) {
          ...
          let dropx, dropy;
          let dropTries = 0;
          let isGoodDrop = false;
          do {
            dropx = Math.floor(Math.random() * (cols-2)) + 1;
            dropy = Math.floor(Math.random() * (rows-2)) + 1;
            dropTries++;
            // Don't drop on path or existing blocks. Plus check edge cases.
            if (!pathCells.has(`${dropx},${dropy}`) && !layout.find(l=>l.x===dropx && l.y===dropy)) {
              isGoodDrop = true;
            }
      ...
      if (ptype === 'bifurcator') {
         heads.push({ x:nx, y:ny, dir: BИФURCATE[cdir][0] });
         heads.push({ x:nx, y:ny, dir: BИФURCATE[cdir][1] });
      } else {
         heads.push({ x:nx, y:ny, dir: nextDir });
      }
    }
    ...
    if (antennas > 0 && pieces.length >= (levelIdx===0 ? 1 : 2)) {
      ...
      const emptyFunc = () => {
         ...
         while(sanity < 100) {
            ...
            if (!gameLayout.find(l => l.x===rx && l.y===ry) && !pathCells.has(`${rx},${ry}`)) {
               return {x:rx, y:ry};
            }
            sanity++;
         }
      ...
      bestLevel = {
        name: `Sector ${levelIdx+1}`,
        cols, rows,
        par: pieces.length,
        hint: 'Guide the beam to all antennas. Portals preserve the beam direction.',
        cells: gameLayout,
        quiz: [
          { q:'What element preserves the direction of the beam and teleports it?', opts:['Reflector','Portal','Bifurcator','Antenna'], correct:1 },
          { q:'How does a Bifurcator modify the laser beam?', opts:['It speeds it up', 'It splits it into two paths', 'It changes its color', 'It stops the beam'], correct:1 },
        ]
      };
      valid = true;
    }
...
function traceBeam(grid, cols, rows) {
  ...
  while (queue.length > 0) {
    ...
    if (!type || type === 'empty') {
      ...
      beamCells.add(cellKey);
      BИФURCATE[dir].forEach(d => queue.push({ x:nx, y:ny, dir:d }));
...
const LaserPuzzleGame = () => {
  ...
  useEffect(() => {
    if (!procLevels) {
      const levels = isDemo ? [generateLevel(0)] : [
        generateLevel(0),
        generateLevel(1),
        generateLevel(2)
      ];
  ...
  useEffect(() => {
    if (procLevels && levelIdx === 0 && Object.keys(grid).length === 0) {
       setGrid(buildGrid(procLevels[0]));
    }
  }, [procLevels, levelIdx, grid]);
  ...
  const finishGame = useCallback((tm) => {
    ...
    stopTracking('game7', efficiency, quizScore);
    setGamePhase('done');
    setTimeout(() => { navigate('/report', { replace:true }); window.scrollTo(0,0); }, 1400);
  }, [procLevels, quizScore, stopTracking, navigate]);
  ...
  const initGame = () => {
    ...
    setLevelIdx(0);
    if (procLevels) setGrid(buildGrid(procLevels[0]));
    setMoves(0);
    setTotalMoves(0);
    setGamePhase('playing');
  };
  ...
  }, [levelIdx, gamePhase, showInstructions, advanceLevel]);
  ...
  const renderCell = (x, y) => {
    ...
    let bg = 'rgba(213,219,245,0.38)';
    let border = '1px solid rgba(150,160,200,0.15)';
    let cursor = 'default';
    let content = null;

    if (type === 'ship') {
      bg = '#4f46e5'; border = '2px solid #6366f1';
      content = <span style={{ color:'white', fontSize:'1.2rem', fontWeight:'900', lineHeight:1 }}>{SHIP_ARROW[cell.dir]}</span>;
    } else if (type === 'rock') {
      bg = '#475569'; border = '1px solid #334155';
      content = <div style={{ width:'72%', height:'72%', background:'#334155', borderRadius:'3px' }} />;
    } else if (type === 'reflector_ne') {
      bg = isSel ? '#bbf7d0' : 'rgba(34,197,94,0.14)';
      border = isSel ? '2px solid #22c55e' : '1px solid rgba(34,197,94,0.4)';
      cursor = 'pointer';
      content = <span style={{ color:'#15803d', fontSize:'1.6rem', fontWeight:'900', lineHeight:1, transform:'rotate(0deg)' }}>/</span>;
    } else if (type === 'reflector_nw') {
      bg = isSel ? '#a7f3d0' : 'rgba(20,184,166,0.14)';
      border = isSel ? '2px solid #14b8a6' : '1px solid rgba(20,184,166,0.4)';
      cursor = 'pointer';
      content = <span style={{ color:'#0d9488', fontSize:'1.6rem', fontWeight:'900', lineHeight:1 }}>\</span>;
    } else if (type === 'bifurcator') {
      bg = isSel ? '#fed7aa' : 'rgba(249,115,22,0.13)';
      border = isSel ? '2px solid #f97316' : '1px solid rgba(249,115,22,0.40)';
      cursor = 'pointer';
      content = <span style={{ color:'#ea580c', fontSize:'1.15rem', fontWeight:'900' }}>⊕</span>;
    } else if (type === 'portal_blue') {
      bg = isSel ? '#c7d2fe' : 'rgba(99,102,241,0.1)';
      border = isSel ? '2px solid #6366f1' : '1px dashed rgba(99,102,241,0.6)';
      cursor = 'pointer';
      content = <span style={{ color:'#4f46e5', fontSize:'1.2rem', fontWeight:'900' }}>⚇</span>;
    } else if (type === 'antenna') {
      bg = isLit ? 'rgba(16,185,129,0.25)' : 'rgba(217,70,239,0.13)';
      border = isLit ? '2px solid #10b981' : '1px solid rgba(217,70,239,0.5)';
      content = <span style={{ fontSize:'1rem' }}>{isLit ? '✓' : '◉'}</span>;
    }
    ...
    return (
      <div
        ...
        style={{
          width: CELL, height: CELL, background: bg, border, cursor,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          position: 'relative', boxShadow: isSel ? '0 0 0 3px rgba(99,102,241,0.5)' : 'none',
      ...
      >
        ...
        {isBeam && !type && (
          <div style={{
            ...
            background:'rgba(251,191,36,0.75)', zIndex:1, pointerEvents:'none',
          }} />
        )}
  ...
  if (showInstructions) {
    return (
      <InstructionInterstitial
        ...
        timeLimit="3 min / level"
        onStart={initGame}
      />
    );
  }

  const allAntennas = Object.keys(grid).filter(k => grid[k].type === 'antenna');
      ...
      <AnimatePresence mode="wait">
        ...
        {(gamePhase === 'playing' || gamePhase === 'levelComplete') && level && (
          <motion.div key={`level-${levelIdx}`} initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0 }} className="glass-panel" style={{ padding:'16px', display:'flex', flexDirection:'column', alignItems:'center', gap:'10px' }}>
            ...
            <div style={{ border:'1px solid rgba(99,102,241,0.2)', borderRadius:'8px', background:'rgba(220,225,255,0.45)', padding:'4px', overflow:'auto' }}>
              <div style={{ display:'grid', gridTemplateColumns:`repeat(${level.cols}, ${CELL}px)`, gap:'2px' }}>
                {Array.from({ length:level.rows }, (_, y) =>
                  Array.from({ length:level.cols }, (_, x) => renderCell(x, y))
                )}
              </div>
            </div>
            ...
            <div style={{ display:'flex', gap:'12px', alignItems:'center', width:'100%', justifyContent:'space-between' }}>
              ...
              <button onClick={handleReset} style={{ background:'rgba(99,102,241,0.1)', border:'1px solid rgba(99,102,241,0.3)', color:'#4f46e5', borderRadius:'8px', padding:'6px 14px', cursor:'pointer', fontSize:'0.8rem', fontWeight:'600', whiteSpace:'nowrap' }}>
                ↺ Reset Level
              </button>
            </div>

            {gamePhase === 'levelComplete' && (
              <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} style={{ padding:'10px 24px', background:'rgba(16,185,129,0.15)', border:'2px solid #10b981', borderRadius:'10px', color:'#059669', fontWeight:'800', fontSize:'0.95rem', textTransform:'uppercase', letterSpacing:'2px' }}>
                ✓ Level Complete!
              </motion.div>
            )}
          </motion.div>
        ...
        {gamePhase === 'quiz' && (
          <motion.div key="quiz" initial={{ scale:0.9, opacity:0 }} animate={{ scale:1, opacity:1 }} className="glass-panel" style={{ padding:'40px', maxWidth:'580px', textAlign:'center' }}>
            ...
            <p style={{ color:'#1e1b4b', marginBottom:'32px', fontSize:'1.05rem', lineHeight:'1.65' }}>{level.quiz[quizStep].q}</p>
            <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
              {level.quiz[quizStep].opts.map((opt, i) => (
