...
const LEVELS = [
  {
    ...
    targets: [
      ...
      { id:3, x:9, y:1, color:'#10b981', points: 50, dropZone:{x:0,y:8} },
    ],
    stations: [],
    energyDrain: 0,
    timeLimit: 90,
  ...
  },
  {
    walls: WALLS_L2,
    targets: [
      { id:4, x:6, y:0, color:'#ef4444', points:150, dropZone:{x:3,y:9} },
      { id:5, x:3, y:4, color:'#10b981', points: 50, dropZone:{x:6,y:3} },
      { id:6, x:0, y:7, color:'#3b82f6', points:100, dropZone:{x:9,y:4} },
    ],
  ...
  {
    walls: WALLS_L3,
    targets: [], // Targets for L3 appear spontaneously
    stations: [{ x:6, y:3 }],
    energyDrain: 4,
    timeLimit: 60,
    startPos: { x:0, y:0 }
...
const GridOptimizerGame = () => {
  ...
  const [sats, setSats] = useState({}); // { targetId: satisfaction 0-100 }
  ...
  const setGS = useCallback((s) => { gameStateRef.current = s; setGameState(s); }, []);

  const updateState = useCallback((u) => {
    stateRef.current = { ...stateRef.current, ...u };
    if ('player' in u) setPlayer({ ...u.player });
    if ('inventory' in u) { setInventory(u.inventory); inventoryRef.current = u.inventory; }
    if ('targets' in u) setTargets([...u.targets]);
    if ('energy' in u) setEnergy(u.energy);
    if ('score' in u) setScore(u.score);
    if ('round' in u) setRound(u.round);
  }, []);
  ...
  const loadLevel = useCallback((idx) => {
    ...
    // We strictly use LEVELS for mapping instead of procLevels
  ...
  }, [updateState, transitionToQuiz, effectiveMaxRounds]);

  useEffect(() => { 
    if (isActive) { 
      if (round === 0 && !hasEndedRef.current && stateRef.current.round === 0) {
        startTracking();
        loadLevel(0);
      }
    } 
  ...
  // Level countdown
  useEffect(() => {
    if (!isActive || gameStateRef.current !== 'playing') return;
    clearInterval(levelTimerRef.current);
  ...
  }, [isActive, round, loadLevel, transitionToQuiz, effectiveMaxRounds]);

  // Per-target satisfaction timer 
  useEffect(() => {
    if (!isActive || gameStateRef.current !== 'playing') return;
    clearInterval(satTimerRef.current);
    satTimerRef.current = setInterval(() => {
      ...
      if (changed) {
        ...
        const expiredTargets = stateRef.current.targets.map(t => {
          if (t.active && (newSats[t.id] ?? 100) <= 0 && (!inv || inv.id !== t.id)) {
            return { ...t, active:false };
          }
          return t;
        });
        const anyExpired = expiredTargets.some((t, i) => t.active !== stateRef.current.targets[i].active);
        if (anyExpired) {
  ...
  // Spontaneous Target Spawner for Round 3
  useEffect(() => {
     if (!isActive || gameStateRef.current !== 'playing' || stateRef.current.round !== 2 || effectiveMaxRounds < 3) return;
     clearInterval(spawnerTimerRef.current);
     
     let targetIdCounter = 100;
     const spawnOptions = [
       { color:'#ef4444', points:150 },
       { color:'#3b82f6', points:100 },
       { color:'#10b981', points:50 }
     ];
  ...
  }, [isActive, round, effectiveMaxRounds]);
  ...
  const move = useCallback((dir) => {
    ...
    if (newEnergy <= 0 && lvl.energyDrain > 0) {
      updateState({ player:{x:nx,y:ny}, energy:0, inventory:newInv, targets:newTargets, score:newScore });
      setFuelEmpty(true);
      clearInterval(levelTimerRef.current);
      clearInterval(spawnerTimerRef.current);
      setTimeout(() => {
    ...
    updateState({ player:{x:nx,y:ny}, energy:newEnergy, inventory:newInv, targets:newTargets, score:newScore });
    
    if (newTargets.every(t=>!t.active) && !newInv) {
      if (r+1 < effectiveMaxRounds) loadLevel(r+1); else transitionToQuiz();
    }
  }, [isActive, updateState, loadLevel, transitionToQuiz, recordError, effectiveMaxRounds]);
  ...
  const finishGame = () => {
    if (hasEndedRef.current) return;
    hasEndedRef.current=true; setGS('done');
    stopTracking('game6', stateRef.current.score, quizScoreRef.current);
    setTimeout(()=>{ navigate('/game/7',{replace:true}); window.scrollTo(0,0); }, 1500);
  };
  ...
  const renderGrid = () => {
    ...
    for (let y=0; y<GRID; y++) {
      for (let x=0; x<GRID; x++) {
        ...
        else if (isDrop) content = (
          <motion.div animate={{ scale:[0.7,1,0.7] }} transition={{ duration:1.2, repeat:Infinity }}
            style={{ width:'38%', height:'38%', borderRadius:'50%', background:`${inventory.color}55`, border:`2px solid ${inventory.color}` }} />
        );
        ...
        if (target) {
          ...
          content = (
            <div style={{ position:'relative', width:'52%', height:'52%', background:target.color, borderRadius:'3px', boxShadow:`0 2px 6px ${target.color}88`, display:'flex', alignItems:'center', justifyContent:'center' }}>
              <span style={{ position:'absolute', top:'-15px', left:'50%', transform:'translateX(-50%)', fontSize:'8px', color:satColor, fontWeight:'bold', whiteSpace:'nowrap' }}>{sat}%</span>
            </div>
          );
        }

        if (isPlayer) content = (
          <div style={{ width:'68%', height:'68%', borderRadius:'4px', background:'#4f46e5', border:'2px solid rgba(255,255,255,0.9)', boxShadow:'0 0 10px rgba(79,70,229,0.7)' }} />
        );

        cells.push(
          <div key={`${x}-${y}`} style={{ width:'36px', height:'36px', background:bg, border, display:'flex', justifyContent:'center', alignItems:'center', position:'relative' }}>
            {content}
          </div>
        );
      }
  ...
  const ArrowBtn = ({ dir, label }) => (
    <button
      ...
      onPointerDown={(e) => {
        ...
        setFlashDir(dir);
        setTimeout(() => setFlashDir(null), 200);
        move(dir);
      }}
      style={{ width:46, height:46,
        background: flashDir === dir ? 'rgba(79,70,229,0.55)' : 'rgba(79,70,229,0.12)',
        border: `1px solid ${flashDir === dir ? '#6366f1' : 'rgba(79,70,229,0.3)'}`,
        borderRadius:'10px', fontSize:'1.1rem', cursor:'pointer',
        display:'flex', alignItems:'center', justifyContent:'center',
        userSelect:'none', touchAction:'none', transition:'background 0.1s, border 0.1s',
        boxShadow: flashDir === dir ? '0 0 12px rgba(99,102,241,0.7)' : 'none',
      }}
    >{label}</button>
  );
  ...
  if (showInstructions) {
    return <InstructionInterstitial type="Logistics & Selective Attention" title="City Optimization Grid"
      ...
      timeLimit="Timed" onStart={() => { setShowInstructions(false); setIsActive(true); }} />;
  }
    ...
    <div style={{ width:'100%', minHeight:'100vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'16px', gap:'10px' }}>
      {gameState === 'playing' && (
        <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} className="glass-panel" style={{ padding:'16px', display:'flex', flexDirection:'column', alignItems:'center', gap:'8px' }}>
          ...
          {fuelEmpty && (
            <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} style={{ padding:'6px 20px', background:'#dc2626', color:'white', borderRadius:'8px', fontWeight:'700', fontSize:'0.9rem' }}>
          ...
          <div style={{ padding:'6px', border:'1px solid rgba(99,102,241,0.2)', borderRadius:'10px', background:'rgba(220,225,255,0.5)' }}>
            <div style={{ display:'grid', gridTemplateColumns:`repeat(${GRID}, 36px)`, gap:'2px' }}>
              {renderGrid()}
            </div>
          </div>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:'10px', padding:'6px 14px', background:'rgba(99,102,241,0.07)', borderRadius:'8px', border:'1px solid rgba(99,102,241,0.18)', fontSize:'0.78rem' }}>
            ...
            {inventory
              ? <><div style={{ width:'11px', height:'11px', background:inventory.color, borderRadius:'2px' }}/><span style={{ color:'#1e1b4b' }}>Deliver to <strong>pulsing zone</strong> — sat: <span style={{color:satColor}}>{sats[inventory.id]??100}%</span></span></>
              : <span style={{ color:'#94a3b8', fontStyle:'italic' }}>Empty — pick up a passenger</span>
            }
          </div>
        ...
        <motion.div initial={{ scale:0.9, opacity:0 }} animate={{ scale:1, opacity:1 }} className="glass-panel" style={{ padding:'40px', maxWidth:'580px', textAlign:'center' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
            {QUIZ[quizStep].opts.map((opt, i) => (
              <button key={i} className="btn" onClick={() => handleQuizAnswer(i)} style={{ padding:'13px 18px', textAlign:'left', display:'flex', gap:'12px' }}>
                <span style={{ opacity:0.65 }}>[{i+1}]</span><span>{opt}</span>
              </button>
            ))}
          </div>
        </motion.div>
