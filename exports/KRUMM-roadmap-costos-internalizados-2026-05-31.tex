...
\textbf{Objetivo:} Pasar de repo local funcional a un staging accesible y verificable.
...
\bottomrule
\end{tabularx}
\callout{KBlue}{\textbf{Comentario.} Con fundador técnico, H1 no debería ser caro en cash. El gasto real es infraestructura y una mínima reserva para pruebas en navegador/dispositivo.}

\subsection{H2 --- Candidate assessment MVP flow}
\textbf{Objetivo:} Permitir que un candidato externo complete la evaluación sin asistencia.
...
\bottomrule
\end{tabularx}
\callout{KBlue}{\textbf{Comentario.} Este hito se puede internalizar casi completo. Lo que sí pagaría es testing con usuarios reales o semi-reales, porque los errores de consentimiento, cámara, abandono y sesión aparecen rápido fuera del entorno del equipo.}

\subsection{H3 --- Recruiter portal + reports/export v1}
\textbf{Objetivo:} Hacer usable el producto para un cliente B2B sin depender del desarrollador.
...
\bottomrule
\end{tabularx}
\callout{KBlue}{\textbf{Comentario.} El gasto externo más valioso aquí no es construir UI; es validar con 2--5 recruiters si entienden los límites del reporte, los caveats y qué NO inferir.}

\subsection{H4 --- Privacy/compliance/security hardening}
\textbf{Objetivo:} Bajar riesgo legal, comercial y técnico antes de candidatos reales.
...
\bottomrule
\end{tabularx}
\callout{KBlue}{\textbf{Comentario.} H4 es el primer hito donde el cash-out no debería comprimirse demasiado. Si hay candidatos reales, legal/privacy y seguridad son parte de la confianza del producto.}

\subsection{H5 --- Pilot instrumentation + baseline calibration}
\textbf{Objetivo:} Aprender con datos reales sin vender el score como predictor validado.
...
\bottomrule
\end{tabularx}
\callout{KBlue}{\textbf{Comentario.} H5 no debería presentarse como validación psicométrica completa. Es baseline inicial, calidad de señal, missingness, diferencias por dispositivo/navegador y documentación honesta de claims permitidos/no permitidos.}

\subsection{H6 --- Pilot launch + customer operations}
\textbf{Objetivo:} Ejecutar pilotos reales con soporte, feedback y triage.
...
\bottomrule
\end{tabularx}
\callout{KBlue}{\textbf{Comentario.} Si H5 ya cubre incentivos de participantes, no duplicar esta partida en H6. Aquí conviene dejar reserva para soporte operativo y bugs en vivo.}

\subsection{H7 --- MVP packaging / private beta comercial}
\textbf{Objetivo:} Convertir el piloto en una beta privada vendible y explicable.
...
\bottomrule
\end{tabularx}
\callout{KBlue}{\textbf{Comentario.} H7 puede ser barato si el equipo fundador hace deck, pricing, landing y demo. El gasto externo sólo se justifica si se quiere acelerar calidad visual/video o paid outreach.}
